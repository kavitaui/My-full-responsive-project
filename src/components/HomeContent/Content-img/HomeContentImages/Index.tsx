
export { default as app } from './app-img1.png'
export { default as app2 } from './app-img2.png'
export { default as app3 } from './app-img3.png'
export { default as applestore } from './apple-store.png'
export { default as altassian1 } from './atlassian1.png'
export { default as bgshape2 } from './bg-shape2.png'
export { default as blacklogo } from './black-logo.png'
export { default as frame } from './frame.png'
export { default as gdrive } from './gdrive.png'
export { default as jira } from './jira.png'
export { default as logo1 } from './logo1.png'
export { default as logo2 } from './logo2.png'
export { default as partner1 } from './partner1.png'
export { default as partner2 } from './partner2.png'
export { default as partner3 } from './partner3.png'
export { default as partner5 } from './partner5.png'
export { default as partner6 } from './partner6.png'
export { default as playstore } from './play-store.png'
export { default as screenshots1 } from './screenshots1.png'
export { default as screenshots2 } from './screenshots2.png'
export { default as screenshots3 } from './screenshots3.png'
export { default as screenshots4 } from './screenshots4.png'
export { default as screenshots5 } from './screenshots5.png'
export { default as shape10 } from './shape10.png'
export { default as shape2 } from './shape2.png'
export { default as skype } from './skype.png'
export { default as slack } from './slack.png'
export { default as user1 } from './user1.jpg'
export { default as user2 } from './user2.jpg'
export { default as user3 } from './user3.jpg'
export { default as user4 } from './user4.jpg'
export { default as video } from './video-bg1.jpg'






const IMAGES = {
 
  shape10: new URL('./shape10.png', import.meta.url).href,
  partner1: new URL('./partner1.png', import.meta.url).href,
  partner2: new URL('./partner2.png', import.meta.url).href,
  partner3: new URL('./partner3.png', import.meta.url).href,
  partner5: new URL('./partner5.png', import.meta.url).href,
  partner6: new URL('./partner6.png', import.meta.url).href,
  video: new URL('./video-bg1.jpg', import.meta.url).href,
  app: new URL('./app-img1.png', import.meta.url).href,
  app2: new URL('./app-img2.png', import.meta.url).href,
  app3: new URL('./app-img3.png', import.meta.url).href,
  playstore: new URL('./play-store.png', import.meta.url).href,
  applestore: new URL('./apple-store.png', import.meta.url).href,
  screenshots1: new URL('./screenshots1.png', import.meta.url).href,
  screenshots2: new URL('./screenshots2.png', import.meta.url).href,
  screenshots3: new URL('./screenshots3.png', import.meta.url).href,
  screenshots4: new URL('./screenshots4.png', import.meta.url).href,
  screenshots5: new URL('./screenshots5.png', import.meta.url).href,
  bgshape2: new URL('./bg-shape2.png', import.meta.url).href,
  jira: new URL('./jira.png', import.meta.url).href,
  skype: new URL('./skype.png', import.meta.url).href,
  slack: new URL('./slack.png', import.meta.url).href,
  gdrive: new URL('./gdrive.png', import.meta.url).href,
  logo1: new URL('./logo1.png', import.meta.url).href,
  atlassian1: new URL('./atlassian1.png', import.meta.url).href,
  user1: new URL('./user1.jpg', import.meta.url).href,
  user2: new URL('./user2.jpg', import.meta.url).href,
  user3: new URL('./user3.jpg', import.meta.url).href,
  user4: new URL('./user4.jpg', import.meta.url).href,

  logo2: new URL('./logo2.png', import.meta.url).href,
  blacklogo: new URL('./black-logo.png', import.meta.url).href,
  frame: new URL('./frame.png', import.meta.url).href,

}
export default IMAGES
